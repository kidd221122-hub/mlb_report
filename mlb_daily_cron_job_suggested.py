...
def get_games_by_date(game_date):
    ...
    for idx, base_game in enumerate(base_games):
        ...
        games_list.append({
            ...
            # 場地因子
            "Park_Factor": get_park_factor(base_game.get("venue", {}).get("name", "")),
...
def get_team_season_batting_from_history(team_id, season=2026, days_back=30):
    """
    從過去 N 天的比賽數據中，累計球隊打擊統計
    
    這是替代方案，因為 /teams/{id}/stats 端點返回 406 錯誤
    
    回傳欄位：
    - TeamId, TeamName
    - Season_Batting_AVG, OBP, SLG, OPS
    - Season_HR, R, RBI, Hits, BB, SO
    - Season_PA (Plate Appearances)
    """
    # 注意：這個函式需要先有歷史數據
    # 建議先運行幾天累積數據後再使用
    # 這裡提供框架，實際使用時需要從 Google Sheet 讀取歷史數據並累計
    print(f"⚠️ get_team_season_batting_from_history 需要從歷史數據累計")
    print(f"   建議先運行 7-14 天累積數據後再使用")
    return pd.DataFrame()
...
if __name__ == "__main__":
    ...
    print("🎬 === 開始執行 MLB 每日數據自動化同步流水線（大小分預測版）===")
    ...
    print("\n📊 資料表結構說明：")
    print("  - team_stats: 比賽基本數據 + 先發投手 + 球隊當場打擊/投球 + 場地因子")
    print("  - starting_pitchers: 先發投手賽季數據（ERA, WHIP, K%, BB%, BABIP）")
    print("  - bullpen_record: 牛棚投手賽季數據（SV, HLD, ERA, WHIP, K-BB%）")
    print("\n💡 預測模型使用建議：")
    print("  1. 從 team_stats 取得場地因子與球隊當場打擊數據")
    print("  2. 從 starting_pitchers 取得先發投手 ERA/WHIP/K-BB%")
    print("  3. 從 bullpen_record 取得牛棚整體實力（用於調整 λ）")
    print("  4. 使用 λ = 基礎 × 攻擊調整 × 投手調整 × 場地因子 計算預期得分")
